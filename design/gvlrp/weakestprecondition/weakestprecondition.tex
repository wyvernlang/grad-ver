...
These handlers are the same in structure as the concrete versions described in Sections~\ref{sec:handling while loops} and~\ref{sec:handling method calls}. The key changes are:
\begin{itemize}
\item
  Permissions are only checked for the concrete parts of gradual formulas appearing as the pre- and post-conditions of the method call and working $\WLP$.
\end{itemize}
The gradual handlers are defined as follows.
