...
\newcommand{\reserved}{\tsf{reserved}}
...
\\
&& e_1 \oplus e_2 &\mt&
  (\Pi \merge \grantedPi(e_1) \frames e_1)
  \\ &&&& \land \
...
\\
&& \cacc(e.f) &\mt&
  \set{ \reserved(x\p.f) \mid x\p \in \aliasesPi(x) }
\\
&& \phi_1 \ast \phi_2 &\mt&
...
&& \cif \ e \ \cthen \ \phi_1 \ \celse \ \phi_2 &\mt&
  ...
  (\set{ \pnot \pi \mid \pi \in \tsf{granted}_\Pi(e)} \ \merge \ \tsf{granted}_\Pi(\phi_2))
\\
&& \cunfolding \ \alpha_C(\vec{e}) \ \cin \ \phi\p &\mt&
  \set{ \tsf{assumed}(\alpha_C(\vec{e})) } \ \merge \
  \tsf{granted}_\Pi(\phi\p)
\end{array}
\end{align*}
