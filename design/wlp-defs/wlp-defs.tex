...
where fields$(C) = \overline{T_i \ f_i}$, $\fphi \div x$ means to transitively expand (in-)equalities ($\odot$) and then removing conjunctive terms containing x, and $\overline{x \neq e_i}$ is a conjunction in $\fphi$. 
